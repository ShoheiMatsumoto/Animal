...
bool CGraphics::Initialize(HWND hWnd, int nWidth, int nHeight, bool bWindow)
{
	...
	// 投資投影
	m_fFovy = FOVY;
	D3DXMatrixPerspectiveFovLH(&m_MatProj, D3DXToRadian(m_fFovy), (float)m_nWidth / m_nHeight, NEAR_CLIP, FAR_CLIP);
	
	// 平行投影
	//D3DXMatrixPerspectiveLH(&m_MatProj, m_nWidth * 0.0001f, m_nHeight * 0.0001f, NEAR_CLIP, FAR_CLIP);
...
// 射影変換セット
void CGraphics::SetProjection(int n)
{
	switch(n)
	{
	case PROJ_PERS:
		// 投資投影
		D3DXMatrixPerspectiveFovLH(&m_MatProj, D3DXToRadian(m_fFovy), (float)SCREEN_WIDTH / (float)SCREEN_HEIGHT, NEAR_CLIP, FAR_CLIP);
		m_pDevice->SetTransform(D3DTS_PROJECTION, &m_MatProj);
		break;

	case PROJ_ORT:
		// 平行投影
		D3DXMatrixOrthoLH(&m_MatProj, m_nWidth, m_nHeight, NEAR_CLIP, FAR_CLIP);
		m_pDevice->SetTransform(D3DTS_PROJECTION, &m_MatProj);
		break;
	}
}

void CGraphics::SetFovy(float fFovy)
{
		m_fFovy = fFovy;
		//D3DXMatrixPerspectiveFovLH(&m_MatProj, D3DXToRadian(m_fFovy), (float)m_nWidth / m_nHeight, NEAR_CLIP, FAR_CLIP);
		//m_pDevice->SetTransform(D3DTS_PROJECTION, &m_MatProj);
...
void CGraphics::SetOrt(float fRate)
{
		m_nWidth = SCREEN_WIDTH * fRate;
		m_nHeight = SCREEN_HEIGHT * fRate;
		//D3DXMatrixOrthoLH(&m_MatProj, m_nWidth, m_nHeight, NEAR_CLIP, FAR_CLIP);
		//m_pDevice->SetTransform(D3DTS_PROJECTION, &m_MatProj);
